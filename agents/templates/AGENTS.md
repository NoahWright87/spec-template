...
Every item in `.todo.md` files is prefixed with one of three emoji markers:

| Marker | Meaning | Grep |
|--------|---------|------|
| ❓ | **Unrefined** — filed but not yet assessed. Needs the refine agent. | `grep '^- ❓'` |
| 💎 | **Refined** — sized, with technical detail. Ready to implement. | `grep '^- 💎'` |
| ⏳ | **Waiting** — blocked on human input. Has a `*(waiting for response, asked YYYY-MM-DD)*` annotation. | `grep '^- ⏳'` |

Lifecycle: intake files items as ❓ → refine changes to 💎 (or ⏳ if it needs info) → implement picks up 💎 items → completed items are removed and promoted to `spec.md`.
