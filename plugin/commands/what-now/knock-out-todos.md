---
description: "Pick the easiest open TODO items and implement them"
argument-hint: "Optional: number of TODOs to tackle (default: 1)"
---

# Knock Out TODOs

Identify and implement the easiest open TODO items in this repository. The user may specify how many to tackle — default is 1 if not stated.

Number to tackle: $ARGUMENTS (default 1 if blank)
...
6. Update `CHANGELOG.md` with the next semver version number (e.g., `## vX.Y.Z`, where `vX.Y.Z` is the next version after the current top entry) and a concise summary of what was done (max ~5 bullets, brief). If any completed items were GH-linked, output a ready-to-paste block at the end of your summary:
