---
description: "Not sure what to do? Assess repo status and get a recommendation"
---
...
Read `specs/.meta.json` if it exists. Look for the key `"what_now_assess"`.

**If the key is missing (even if the file exists):**
...
Save their answer by adding the key to `specs/.meta.json` (creating the file if needed, preserving any existing keys):
- "Always assess" → `"what_now_assess": "auto"`
- "Only when I ask" → `"what_now_assess": "on_demand"`

Then proceed based on what they chose.
...
| Option label | Description | Slash command |
|-------------|-------------|--------------|
| Review an open PR | Self-review your diff, leave explanatory comments, respond to Copilot and reviewer comments | `/what-now:pr-review` |
| File ideas and GitHub Issues | Sort new ideas, feature requests, and open GH Issues into the right TODO spec files | `/what-now:intake` |
| Add detail to TODO items | Clarify vague TODOs, add effort estimates (XS–XL), and open a PR with proposed spec updates | `/what-now:refine` |
| Implement TODO items | Pick the easiest open TODO items and implement them | `/what-now:knock-out-todos` |
| Backfill specs from existing code | Generate spec files from an existing codebase; mark gaps with `> **TODO:**` for later | `/what-now:spec-backfill` |
...
Once the user selects an option, invoke the corresponding slash command. Do not invoke any command until the user has made their choice — only run the one they pick.
